 ...
 */
async startPlaylistScan(url: string, scanId: number) : Promise<Result<null, AppError>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("start_playlist_scan", { url, scanId }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async cancelPlaylistScan(scanId: number) : Promise<Result<null, AppError>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("cancel_playlist_scan", { scanId }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
export type PlaylistScanEvent = { scanId: number; eventType: string; 
/**
 * Present only on the first "entries" event of a scan.
 */
meta: PlaylistScanMeta | null; entries: PlaylistEntry[]; 
/**
 ...
 */
export type PlaylistScanMeta = { playlistId: string; title: string; 
/**
 * Normalized URL actually scanned (bare channel URLs get a `/videos` suffix).
 */
url: string; channelName: string | null; 
/**
