<script lang="ts">
  ...
  // "Records" section — history table (completed downloads), searchable + paginated.
  ...
  // completed는 전용 'Completed' 탭에서만 노출하고 기본(전체) 목록에는 섞지 않는다.
  ...
  onMount(async () => {
    ...
    interval = setInterval(async () => {
      await loadActive()
      // Keep records fresh as downloads complete, but don't fight an active search/paging.
