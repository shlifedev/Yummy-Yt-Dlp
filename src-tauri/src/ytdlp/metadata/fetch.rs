...
pub(super) fn normalize_bare_channel_url(url: &str) -> String {
...
/// Map one `--flat-playlist --dump-json` NDJSON object to a `PlaylistEntry`.
pub(super) fn parse_flat_entry(entry: &serde_json::Value) -> Option<PlaylistEntry> {
...
/// Extract playlist-level metadata from the first flat-dump entry. `playlist_count` is often
/// absent under `--lazy-playlist`, so `video_count` stays `None` until the scan completes.
pub(super) fn extract_scan_meta(first_entry: &serde_json::Value, url: &str) -> PlaylistScanMeta {
    ...
    PlaylistScanMeta {
        ...
        url: url.to_string(),
        channel_name,
        video_count: first_entry["playlist_count"].as_u64(),
    }
}
...
#[cfg(test)]
mod tests {
    use super::{normalize_bare_channel_url, parse_flat_entry};

    #[test]
    fn flat_entry_prefers_full_urls() {
        ...
        assert_eq!(parsed.title.as_deref(), Some("Some video"));
        assert_eq!(parsed.duration, Some(61));
    }

    #[test]
    fn flat_entry_reconstructs_youtube_id_only_entries() {
        let entry = serde_json::json!({ "id": "abc123", "ie_key": "Youtube" });
        let parsed = parse_flat_entry(&entry).unwrap();
        assert_eq!(parsed.url, "https://www.youtube.com/watch?v=abc123");
    }
    ...
    #[test]
    fn flat_entry_without_url_or_id_is_skipped() {
        let entry = serde_json::json!({ "title": "broken" });
        assert!(parse_flat_entry(&entry).is_none());
    }

    #[test]
    fn flat_entry_falls_back_to_thumbnails_array() {
        let entry = serde_json::json!({
            "id": "abc123",
            "ie_key": "Youtube",
